...
function createEditorTabId(seed: number): string {
  return `tab-${Date.now()}-${seed}-${Math.random().toString(36).slice(2, 8)}`;
}

/** Options used to bind editor-tab state to host app services. */
export interface EditorTabsSliceOptions {
  cancelExecution?: (tabId: string) => void;
}

/**
 * Creates the editor tab slice with pluggable cancellation behavior so the
 * editor package stays independent from the host execution engine.
 */
export const createEditorTabsSlice = (
  options: EditorTabsSliceOptions = {}
): import('zustand').StateCreator<EditorTabsState> => {
  const { cancelExecution = () => undefined } = options;
  ...
  return (set) => ({
    ...
    setTabPendingRun: (id, isPendingRun) =>
      set((state) => ({
        tabs: state.tabs.map((tab) =>
          tab.id === id ? { ...tab, isPendingRun } : tab
        ),
      })),

    setTabResults: (id, results) => {
      const limited =
        results.length > MAX_RESULTS ? results.slice(-MAX_RESULTS) : results;
      set((state) => ({
        tabs: state.tabs.map((tab) =>
          tab.id === id ? { ...tab, result: limited } : tab
        ),
      }));
    },

    appendTabResult: (id, resultItem) =>
      set((state) => ({
        tabs: state.tabs.map((tab) => {
    ...
    setTabPromptRequest: (id, message, type = 'text', executionId = null) =>
      set((state) => ({
        tabs: state.tabs.map((tab) =>
          tab.id === id
            ? {
                ...
                promptType: type,
                promptExecutionId: message
                  ? (executionId ?? tab.promptExecutionId ?? id)
                  : null,
              }
            : tab
        ),
