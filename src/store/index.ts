    ...
    set((state: TState) => {
      ...
      const nextSlice =
        typeof partial === 'function'
          ? (partial as (s: TSlice) => TSlice | Partial<TSlice>)(currentSlice)
          : partial;
      return {
        ...state,
        [sliceKey]: replace
          ? nextSlice
          : { ...(state[sliceKey] as object), ...(nextSlice as object) },
  ...
  subscribeWithSelector(
    persist(
      (set, get) => ({
        ...
        chat: createNestedSlice(
          set as never,
          get as never,
          'chat',
          createChatSlice
        ),
        aiSettings: createNestedSlice(
          set as never,
          get as never,
          'aiSettings',
          createAISettingsSlice
        ),
        rag: createNestedSlice(
          set as never,
          get as never,
          'rag',
          createRagSlice
