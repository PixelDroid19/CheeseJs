...
export const useCodeStore = (<T>(selector?: (state: CodeState) => T) => {
  return useAppStore(
    selector
      ? (state) => selector(state.code)
      : (state) => state.code as unknown as T
  );
...
export const useSettingsStore = (<T>(
  ...
  return useAppStore(
    selector
      ...
      : (state) => state.settings as unknown as T
  );
...
export const useLanguageStore = (<T>(
  ...
  return useAppStore(
    selector
      ? (state) => selector(state.language)
      : (state) => state.language as unknown as T
  );
...
export const useHistoryStore = (<T>(selector?: (state: HistoryState) => T) => {
  return useAppStore(
    selector
      ? (state) => selector(state.history)
      : (state) => state.history as unknown as T
  );
}) as StoreHook<HistoryState>;

export const useSnippetsStore = (<T>(
  selector?: (state: SnippetsState) => T
) => {
  return useAppStore(
    selector
      ? (state) => selector(state.snippets)
      : (state) => state.snippets as unknown as T
  );
...
export const useChatStore = (<T>(selector?: (state: ChatState) => T) => {
  return useAppStore(
    selector
      ? (state) => selector(state.chat)
      : (state) => state.chat as unknown as T
  );
}) as StoreHook<ChatState>;

export const useAISettingsStore = (<T>(
  selector?: (state: AISettingsState) => T
) => {
  return useAppStore(
    selector
      ? (state) => selector(state.aiSettings)
      : (state) => state.aiSettings as unknown as T
  );
...
export const useRagStore = (<T>(selector?: (state: RagState) => T) => {
  return useAppStore(
    selector
      ? (state) => selector(state.rag)
      : (state) => state.rag as unknown as T
  );
}) as StoreHook<RagState>;

export const usePackagesStore = (<T>(
  selector?: (state: PackagesState) => T
) => {
  return useAppStore(
    selector
      ? (state) => selector(state.packages)
      : (state) => state.packages as unknown as T
  );
}) as StoreHook<PackagesState>;

export const usePythonPackagesStore = (<T>(
  selector?: (state: PythonPackagesState) => T
) => {
  return useAppStore(
    selector
      ? (state) => selector(state.pythonPackages)
      : (state) => state.pythonPackages as unknown as T
  );
}) as StoreHook<PythonPackagesState>;

export const useEditorTabsStore = (<T>(
  selector?: (state: EditorTabsState) => T
) => {
  return useAppStore(
    selector
      ? (state) => selector(state.editorTabs)
      : (state) => state.editorTabs as unknown as T
  );
}) as StoreHook<EditorTabsState>;

export const useLspStore = (<T>(selector?: (state: LspState) => T) => {
  return useAppStore(
    selector
      ? (state) => selector(state.lsp)
      : (state) => state.lsp as unknown as T
  );
}) as StoreHook<LspState>;

function attachZustandMethods<T>(hook: StoreHook<T>, sliceKey: keyof AppState) {
          ...
          [sliceKey]: replace
            ...
            : {
                ...
                ...(typeof partial === 'function'
                  ? (partial as (s: T) => Partial<T> | T)(
                      state[sliceKey] as unknown as T
                    )
                  : partial),
              },
...
attachZustandMethods(useLspStore, 'lsp');
