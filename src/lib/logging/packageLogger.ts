...
class PackageLogger {
  ...
  private trimLogs(): void {
    if (this.installationLogs.length > this.config.maxEntries) {
      this.installationLogs = this.installationLogs.slice(
  ...
  logInstallStart(
    ...
    this.consoleLog(
      'info',
      `Installing package: ${packageName} (attempt ${attempt}/${maxAttempts})`
    );
  ...
  logInstallSuccess(
    ...
    this.consoleLog(
      'info',
      `Package installed successfully: ${packageName}${version ? `@${version}` : ''}`,
  ...
  logAutoRunPending(packages: string[]): string {
    ...
    this.consoleLog('info', `Auto-run pending after package installation`, {
      packages,
    });
  ...
  logAutoRunStart(
    triggeredBy: AutoRunLogEntry['triggeredBy'] = 'package-install'
  ): string {
    if (!this.shouldLog('info')) return '';
  ...
  getRecentLogs(count: number = 10): {
    installation: InstallationLogEntry[];
    autoRun: AutoRunLogEntry[];
  } {
    return {
...
export const logInstallStart =
  packageLogger.logInstallStart.bind(packageLogger);
export const logInstallSuccess =
  packageLogger.logInstallSuccess.bind(packageLogger);
export const logInstallFailure =
  packageLogger.logInstallFailure.bind(packageLogger);
export const logAutoRunPending =
  packageLogger.logAutoRunPending.bind(packageLogger);
export const logAutoRunStart =
  packageLogger.logAutoRunStart.bind(packageLogger);
export const logAutoRunComplete =
  packageLogger.logAutoRunComplete.bind(packageLogger);
export const logAutoRunSkipped =
  packageLogger.logAutoRunSkipped.bind(packageLogger);
export const logAutoRunFailed =
  packageLogger.logAutoRunFailed.bind(packageLogger);
