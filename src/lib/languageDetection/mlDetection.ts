...
export async function initializeMLModel(): Promise<void> {
    ...
    modelLoadPromise = (async () => {
        try {
            ...
            console.log('[MLDetection] ML model loaded');
        } catch (error) {
            console.warn(
...
export async function detectWithML(
    ...
    for (const pattern of DEFINITIVE_PYTHON_PATTERNS) {
        if (pattern.test(trimmed)) {
            console.log(`[MLDetection] Definitive Python: ${pattern}`);
    ...
    for (const pattern of DEFINITIVE_TS_PATTERNS) {
        if (pattern.test(trimmed)) {
            console.log(`[MLDetection] Definitive TypeScript: ${pattern}`);
            const result: DetectionResult = {
                monacoId: 'typescript',
                confidence: 0.98,
                isExecutable: true,
    ...
    for (const pattern of DEFINITIVE_JS_PATTERNS) {
        if (pattern.test(trimmed)) {
            console.log(`[MLDetection] Definitive JS: ${pattern}`);
            const result: DetectionResult = {
                monacoId: 'javascript',
                confidence: 0.98,
                isExecutable: true,
    ...
    if (!modelOperations) {
        console.warn('[MLDetection] ML model not available, using pattern fallback');
        const result = patternBasedDetection(content);
        updateCache(cacheKey, result);
        onConfidenceUpdate?.(result.confidence);
        return result;
