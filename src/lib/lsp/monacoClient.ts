...
export function isLspClientActive(langId: string): boolean {
  return !!activeClients[langId];
}

export async function startLspClient(
  langId: string,
  _documentSelector: string[]
) {
  if (activeClients[langId]) {
    return;
  }

  // Ask the Electron backend to spawn the LSP server process
  const res = await window.lspBridge.start(langId);
  if (!res.success) {
    throw new Error(`Failed to start LSP backend: ${res.error}`);
  }

  // Set up JSON-RPC connection directly over IPC
  const reader = new IpcMessageReader(langId);
  const writer = new IpcMessageWriter(langId);
  const connection = jsonrpc.createMessageConnection(reader, writer);

  connection.listen();

  // Send 'initialize' request to the language server
  // Fetch the full config to pass initializationOptions if available
  let initializationOptions: unknown = undefined;
  try {
    const fullConfig = (await window.lspConfig?.getConfig()) as
      | LspConfigResponse
      | undefined;
  ...
  // Map Monaco language to LSP language ID
  ...
  // For JS/TS, Monaco's built-in TypeScript worker already provides excellent
  // completions, hover, and signature help. Only register LSP providers for
  // other languages where Monaco has no built-in intelligence.
  const hasBuiltInIntelligence =
    langId === 'javascript' || langId === 'typescript';

  // ========================
  // Completion Provider (non-JS/TS only)
  // ========================
  ...
  // ========================
  // Signature Help Provider (non-JS/TS only)
  // ========================
  if (!hasBuiltInIntelligence) {
  ...
  // Track models for didOpen/didChange
  const openDocs = new Set<string>();

  // Listen for model content changes to send didChange
...
export function stopLspClient(langId: string) {
  ...
  try {
    window.lspBridge.stop(langId);
  } catch (e) {
    console.warn(`Error stopping LSP bridge for ${langId}:`, e);
  }
}
...
function mapCompletionKind(kind?: number): monaco.languages.CompletionItemKind {
  // LSP CompletionItemKind -> Monaco CompletionItemKind (roughly 1:1 mapping)
