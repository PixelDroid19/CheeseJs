...
export class ExecutionThrottler<T = unknown> {
    ...
    private calculateAdaptiveDelay(): number {
        ...
        const adaptiveDelay = this.config.minDelay + (avgTime * this.config.adaptiveWeight)

        return Math.min(Math.max(adaptiveDelay, this.config.minDelay), this.config.maxDelay)
    }
    ...
    async schedule(
        ...
        this.metrics.averageDelay =
            (this.metrics.averageDelay * (this.metrics.executedRequests || 1) + delay) /
    ...
    getMetrics(): ThrottlerMetrics & {
        ...
        const totalHandled = this.metrics.executedRequests + this.metrics.cancelledRequests
        return {
            ...this.metrics,
            executionRate: totalHandled > 0
                ? this.metrics.executedRequests / totalHandled
                : 0,
