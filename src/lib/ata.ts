    ...
    if (pkg.isInstalled && !fetchedTypes.has(pkg.name)) {
      ...
      if (!success) {
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[ATA] Could not find types for ${pkg.name}, adding fallback.`)
        }
...
async function fetchAndAddTypes (monaco: Monaco, packageName: string, version?: string): Promise<boolean> {
  try {
