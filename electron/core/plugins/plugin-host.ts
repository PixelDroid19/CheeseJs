...
import { pluginLoader, type PluginManifest, type PluginInfo } from './plugin-loader.js';
import { eventBus, ExtensionEvents } from '../../../src/core/events/EventBus.js';
...
export class PluginHost {
    ...
    async initialize(): Promise<void> {
        ...
        this.subscriptions.push(
            eventBus.on(ExtensionEvents.EXTENSION_ERROR, (data: any) => {
                log.error(`[PluginHost] Extension error: ${data.error}`, data);
                if (data.id) {
                    pluginLoader.setPluginStatus(data.id, 'error', data.error);
        ...
        try {
            ...
            for (const plugin of plugins) {
                try {
                    ...
                    log.error(`[PluginHost] Failed to auto-activate plugin ${plugin.manifest.id}:`, e);
                }
            }
        } catch (e) {
     ...
     */
    setMainWindow(window: BrowserWindowLike): void {
    ...
    async activatePlugin(id: string): Promise<void> {
        ...
        try {
            ...
            eventBus.emit(ExtensionEvents.EXTENSION_ERROR, { id, error: errorMessage });
    ...
    async deactivatePlugin(id: string): Promise<void> {
        ...
        try {
            ...
            this.unregisterDeclarativeContributions(id);
    ...
    private async handlePluginReload(pluginId: string): Promise<void> {
        ...
        if (!newInfo) {
            log.error(`[PluginHost] Failed to reload manifest for ${pluginId}, aborting reactivation`);
            return;
        }
    ...
    private registerDefaultExtensionPoints(): void {
        const points = [
            { id: 'commands', description: 'Contribute new commands to the command palette' },
        ...
        for (const point of points) {
            extensionPointRegistry.registerExtensionPoint(point.id, point.description);
        }
    }
     ...
     */
    private registerDeclarativeContributions(manifest: PluginManifest): void {
     ...
     */
    private unregisterDeclarativeContributions(pluginId: string): void {
    ...
    private registerIpcHandlers(): void {
        ...
        ipcMain.handle('plugins:activate', async (_event: Electron.IpcMainInvokeEvent, id: string) => {
            await this.activatePlugin(id);
            return { success: true };
        });

        ipcMain.handle('plugins:deactivate', async (_event: Electron.IpcMainInvokeEvent, id: string) => {
            await this.deactivatePlugin(id);
            return { success: true };
        });

        ipcMain.handle('plugins:install', async (_event: Electron.IpcMainInvokeEvent, sourcePath: string) => {
            const pluginInfo = await pluginLoader.installFromPath(sourcePath);
            return { success: true, plugin: pluginInfo };
        });

        ipcMain.handle('plugins:install-from-url', async (_event: Electron.IpcMainInvokeEvent, url: string, pluginId?: string) => {
            ...
            if (result.success) {
                ...
                try {
                    ...
                    const installed = plugins.find(p => result.path?.includes(p.manifest.id));
                    if (installed) {
                        await this.activatePlugin(installed.manifest.id);
                    }
                } catch (e) {
                    log.warn('[PluginHost] Failed to auto-activate installed plugin:', e);
                }
                this.notifyPluginStateChange();
            }
            return result;
        ...
        ipcMain.handle('plugins:uninstall', async (_event: Electron.IpcMainInvokeEvent, id: string) => {
            await this.deactivatePlugin(id);
        ...
        ipcMain.handle('plugins:read-file', async (_event: Electron.IpcMainInvokeEvent, filePath: string) => {
