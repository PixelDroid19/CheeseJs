...
export class SmartScriptCache {
    ...
    logMetrics(): void {
        ...
        console.log(`[SmartScriptCache] Metrics:`, {
            hitRate: `${(metrics.hitRate * 100).toFixed(1)}%`,
            hits: metrics.hits,
            misses: metrics.misses,
            evictions: metrics.evictions,
            cacheSize: metrics.size,
            memoryUsed: `${(metrics.currentMemory / 1024 / 1024).toFixed(2)}MB`,
            avgAccessTime: total > 0
                ? `${(metrics.totalAccessTime / total).toFixed(2)}ms`
                : 'N/A'
        })
    }
}
...
export function getScriptCache(options?: SmartScriptCacheOptions): SmartScriptCache {
