    ...
    int restore_frame)
{
    ...
    // Avoid DBus single-instance handoff in dev/test shells; handoff can return
    // before GTK emits activate, which exits the native process without a window.
...
static gboolean zero_native_activate_idle(gpointer data) {
    zero_native_gtk_host_t *host = data;
    g_application_activate(G_APPLICATION(host->app));
    g_application_release(G_APPLICATION(host->app));
...
void zero_native_gtk_run(zero_native_gtk_host_t *host, zero_native_gtk_event_callback_t callback, void *context) {
    ...
    char *argv[] = { host->app_name, NULL };
    g_application_hold(G_APPLICATION(host->app));
    g_idle_add(zero_native_activate_idle, host);
    g_application_run(G_APPLICATION(host->app), 1, argv);
}
